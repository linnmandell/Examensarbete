...
if number_of_subplots>=1 && number_of_subplots<=30
	...
	if strcmpi(flag,'newfigure') % if to open new figure
		hcf = figure;
		xSize = 13; %was originaly set to 11
		ySize = 5+5*sqrt(number_of_subplots);
		xLeft = (21-xSize)/2; 
        yTop = (30-ySize)/2;
		set(hcf,'PaperPosition',[xLeft yTop xSize ySize]) 
%         set(hcf,'PaperPosition',[xLeft yTop xSize ySize*0.5]) %for 2 thin subplots
		un=get(0,'units');
		set(0,'units','pixels');
		sz=get(0,'screensize');
		xx=min(min(700,sz(3))/xSize,min(900,sz(4))/ySize); % figure at least 600 wide or 900 height but not outside screen
% 		xx=min(min(600,sz(3))/xSize,min(700,sz(4))/ySize); % figure at
%		least 600 wide or 900 height but not outside screen for thin 2
%		subplots

        set(hcf,'Position',[10 10 xSize*xx ySize*xx])
%         set(hcf,'Position',[10 10 xSize*xx 0.5*ySize*xx]) %for 2 thin subplots
		set(0,'units',un);
...
%     all_axis_position=[0.17 0.2 0.9 0.9]; % xmin ymin xmax ymax for 2
    %subplots thin
